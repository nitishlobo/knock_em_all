...
void Bowling::Ball(int pins_knocked) {
	...
	game_history[balls_bowled_].frame = current_f_;
	game_history[balls_bowled_].pins = pins_knocked;

	if (balls_bowled_ == 0) {
		//Score for first ball
		game_history[balls_bowled_].score = pins_knocked;
	} else {
		//Cumulative ball score
		game_history[balls_bowled_].score = game_history[balls_bowled_ - 1].score + pins_knocked;
	}

	if (balls_bowled_ > 1) {
		//Spare
		//If previous bowl and the bowl before that are in the same frame and add to 10 and it's not in the last frame.
		if ((game_history[balls_bowled_ - 1].frame == game_history[balls_bowled_ - 2].frame)
			&& (game_history[balls_bowled_ - 1].pins + game_history[balls_bowled_ - 2].pins == 10)
			&& (game_history[balls_bowled_ - 2].frame != 9)) {
			//For a spare, add current ball score to previous frame score.
			game_history[balls_bowled_ - 1].score += pins_knocked;
		...
		if ((game_history[balls_bowled_ - 2].pins == 10) && (game_history[balls_bowled_ - 2].frame != 9)) {
			//Add the next two ball scores.
			game_history[balls_bowled_ - 2].score += game_history[balls_bowled_ - 1].pins;
			game_history[balls_bowled_ - 2].score += game_history[balls_bowled_].pins;

			//Recalculate the next two ball scores.
			game_history[balls_bowled_ - 1].score = game_history[balls_bowled_ - 2].score + game_history[balls_bowled_ - 1].pins;
			game_history[balls_bowled_].score = game_history[balls_bowled_ - 1].score + pins_knocked;
		}
	}
	...
	if (balls_bowled_ > 1) {
		if ((current_f_ != 9)
			&& (game_history[balls_bowled_ - 1].frame == game_history[balls_bowled_ - 2].frame)) {
			current_f_++;
		}
	}
}
...
int Bowling::GetScore() {
	return game_history[balls_bowled_ - 1].score;
}
...
int Bowling::GetFramesBowled() {
	return game_history[balls_bowled_ - 1].frame + 1;
}
...
bool Bowling::IsGameOver() {
	...
	if (balls_bowled_ > 2) {
		//game_history is over if the last three frames recorded are 9 (i.e. 10th frame).
		if ((game_history[balls_bowled_ - 1].frame == 9) && (game_history[balls_bowled_ - 2].frame == 9) && (game_history[balls_bowled_ - 3].frame == 9)) {
			over = true;
		} else if ((game_history[balls_bowled_ - 1].frame == 9) && (game_history[balls_bowled_ - 2].frame == 9)) {
			//game_history is also over if the last two frames are recorded are 9 and
			//		there are no spares
			//		nor strikes
			if (((game_history[balls_bowled_ - 1].frame != 10) && (game_history[balls_bowled_ - 2].frame != 10)) || (game_history[balls_bowled_ - 1].frame + game_history[balls_bowled_ - 2].frame != 10)) {
				over = true;
			}
		}
	}
